...
#define UFCR_RXTL_MASK	0x3F	/* RX FIFO trigger level bit mask */
...
static void imx_uart_dma_exit(struct imx_port *sport)
{
	if (sport->dma_chan_rx) {
		...
		/* Not freeing this memory on purpose. We are aware that this causes a memory leak.
		 * This is executed when we
		 * - suspend
		 * - unload the driver
		 * We (BSH) never unload the driver
		 * If we suspend we have had situations in which we no longer
		 * could reallocate this memory buffer due to memory fragmentation
		 * Therefore we just keep this.
		 * It would be nicer if we could differentiate between suspending
		 * and unloading, but this would be a huge rewrite */
		/*
		if (sport->rx_buf.buf) {
		   ...
		   struct ktermios *old)
{
	...
	if (sport->dte_mode)
		ufcr |= UFCR_DCEDTE;
	/* set RX FIFO trigger to one byte if bytes should be pushed instantly to line discipline */
	if (ttyport->push_instantly) {
		ufcr &= ~UFCR_RXTL_MASK;
		ufcr |= RXTL;
	}
...
static int serial_imx_suspend(struct platform_device *dev, pm_message_t state)
{
	...
	/* BSH: The IPG clock is disabled in uart_suspend_port(). If this clock is 
	 * disabled, some interrupt bits cannot be cleared in imx_int() and therefore 
	 * an interrupt can be retriggered all the time. 
	 * The original suspend sequence was to first enable the wakeup from UART
	 * interrupt, call uart_suspend_port() and afterwards do disable_irq(). 
	 * This leads to a deadlock when suspending, because an interrupt could occur
	 * when the IPG clock was already disabled. The function disable_irq() then 
	 * waits infinitely until the interrupt was processed. */
	uart_suspend_port(&imx_reg, &sport->port);
	clk_prepare_enable(sport->clk_ipg);

	disable_irq(sport->port.irq);

	/* BSH: Only wakeup from console UART */
	if (uart_console(&sport->port)) {
