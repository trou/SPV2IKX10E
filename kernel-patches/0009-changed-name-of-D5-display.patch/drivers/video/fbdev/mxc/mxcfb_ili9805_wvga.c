...
#define DISPLAY_POWER_PIN		 186
#define STANDBY_PIN				19
#define RESET_PIN				142
...
static struct fb_videomode truly_lcd_modedb[] = {
	{
	 "TRULY-WVGA", 62, 480, 800, 38128,
	 36, 10,
	 10, 2,
	 2, 1,
/*	 FB_SYNC_OE_LOW_ACT */ 0,
...
/* call this function to put the display GPIOs to ON mode.
 * This function can be called after mipid_prepare_and_init_gpios
 * has been called once.
 *
 *
 */
void mipid_init_gpios(struct mipi_dsi_info *mipi_dsi)
{
...
void mipid_deinit_gpios(struct mipi_dsi_info *mipi_dsi)
{
	gpio_direction_output(DISPLAY_POWER_PIN, 1);		// This is inverted, but we might have some problems as the driver is not driven to 0V but remains at ~1.5V
	gpio_direction_output(STANDBY_PIN, 0);
	gpio_direction_output(RESET_PIN, 0);

	msleep(120);										// per definition, the driver needs to be powerless for 120ms to be considered powered off
	mipi_dsi->lcd_inited = 0;							// mark that we need reinitialization
}

static int mipid_prepare_and_init_gpios(struct mipi_dsi_info *mipi_dsi)
{
	int ret;
	static bool already_requested;
...
int mipid_ili9805_lcd_setup(struct mipi_dsi_info *mipi_dsi)
{
	...
	err = mipid_prepare_and_init_gpios(mipi_dsi);
	CHECK_RETCODE(err);
...
//	DCS_LCMD_PARAM2(0xEB, 0x00, 0x0F); 			 // DSI Lanes Control, one 1 lane
	...
	DCS_LCMD_PARAM3(0xB1, 0x00, 0x12, 0x14);			// Frame Rate Control 1, should result in 61.7 Hz
	msleep(10);
