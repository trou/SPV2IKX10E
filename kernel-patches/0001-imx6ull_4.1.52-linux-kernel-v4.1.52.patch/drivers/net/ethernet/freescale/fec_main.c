...
static int fec_disabled = 0;
static int fec_regulator_enable(struct fec_enet_private *fep, struct regulator *regulator);
static int fec_regulator_disable(struct fec_enet_private *fep, struct regulator *regulator);
...
static void fec_reset_phy(struct platform_device *pdev)
{
	...
	if(fec_disabled)
	{
		/* Don't go any further if we have disabled ETH completely */
		return;
	}
...
static int fec_regulator_enable(struct fec_enet_private *fep, struct regulator *regulator)
{
	...
	mutex_lock(&fep->status_lock);

	if( (fep->active == 1) && ( ! regulator_is_enabled(fep->reg_phy)) )
	{
...
static int fec_regulator_disable(struct fec_enet_private *fep, struct regulator *regulator)
{
	...
	if(regulator_is_enabled(fep->reg_phy))
	{
		ret = regulator_disable(fep->reg_phy);
		if (ret) {
			dev_err(&fep->pdev->dev,
				"Failed to disable phy regulator: %d\n", ret);
		}
	}
...
static ssize_t fec_show_status(struct device *dev,
			       struct device_attribute *attr, char *buf)
{
	struct net_device *ndev = dev_get_drvdata(dev);
	struct fec_enet_private *fep = netdev_priv(ndev);

	return sprintf(buf, "%d\n", regulator_is_enabled(fep->reg_phy));
}

static ssize_t fec_set_status(struct device *dev,
				...
				const char *buf, size_t count)
{
	...
	if( ret == 1 )
	{
		...
		{
			fep->active = 1;
			ret = fec_regulator_enable(fep, fep->reg_phy);
			if (ret) {
				dev_err(&fep->pdev->dev,
					"Failed to enable phy regulator: %d\n", ret);
			}
		...
		{
			fep->active = 0;
			ret = fec_regulator_disable(fep, fep->reg_phy);
			if (ret) {
				dev_err(&fep->pdev->dev,
					"Failed to disable phy regulator: %d\n", ret);
			}
	...
	{
		dev_err(&fep->pdev->dev,
			"ERROR:%s - Illegal parameter (%d) !\n", __func__, ret);
	}

	return count;
}

static DEVICE_ATTR(status, 0644, fec_show_status, fec_set_status);
...
failed_init:
	if (fep->reg_phy)
		fec_regulator_disable(fep, fep->reg_phy);
failed_regulator:
	fec_enet_clk_enable(ndev, false);
failed_clk:
failed_phy:
...
static int __init fec_disable(char *str)
{
	if (!strcmp(str, "1"))
		fec_disabled = 1;
...
__setup("ethernet_disable=", fec_disable);
